...
const THEMES = {
  ...
  mansion: {
    ...
    voices: [
      { // 静かな低音
        type: "triangle", vol: 0.040,
        notes: [[0, 110, 2], [2, 110, 2], [4, 87, 2], [6, 98, 2], [8, 110, 2], [10, 131, 2], [12, 98, 2], [14, 82, 2]],
      },
      { // 硝子細工の旋律
        type: "sine", vol: 0.032,
        notes: [[0, 440, 2], [2, 523, 2], [4, 494, 2], [6, 440, 2], [8, 392, 2], [10, 440, 1], [11, 523, 1], [12, 494, 4]],
      },
    ],
  },
  // 酒場: 陽気で速い踊りの曲
  tavern: {
    ...
    voices: [
      { // 跳ねるベース
        ...
        notes: [[0, 131, 1], [2, 196, 1], [4, 131, 1], [6, 196, 1], [8, 147, 1], [10, 220, 1], [12, 131, 1], [14, 196, 1]],
      },
      { // 笛の踊り
        type: "square", vol: 0.026,
        notes: [[0, 523, 1], [1, 587, 1], [2, 659, 2], [4, 523, 1], [5, 587, 1], [6, 659, 2],
                [8, 698, 1], [9, 659, 1], [10, 587, 1], [11, 523, 1], [12, 587, 2], [14, 523, 2]],
      },
    ],
  },
  // 商店: 軽快な商いのリズム
  shop: {
    ...
    voices: [
      { // 刻むベース
        ...
        notes: [[0, 147, 2], [2, 110, 2], [4, 147, 2], [6, 110, 2], [8, 165, 2], [10, 123, 2], [12, 147, 2], [14, 110, 2]],
      },
      { // 明るい呼び込み
        type: "square", vol: 0.024,
        notes: [[0, 587, 1], [1, 659, 1], [2, 740, 2], [4, 659, 2], [6, 587, 2],
                [8, 494, 1], [9, 587, 1], [10, 659, 2], [12, 587, 2], [14, 494, 2]],
      },
    ],
  },
  // 宿屋: 静かな子守唄
  inn: {
    ...
    voices: [
      { // まどろむ低音
        ...
        notes: [[0, 98, 4], [4, 110, 4], [8, 87, 4], [12, 98, 4]],
      },
      { // 囁く旋律
        type: "sine", vol: 0.030,
        notes: [[0, 392, 2], [2, 440, 2], [4, 494, 4], [8, 440, 2], [10, 392, 2], [12, 330, 4]],
      },
    ],
  },
  // 王宮: 荘厳な行進
  palace: {
    ...
    voices: [
      { // 重厚な低音
        type: "triangle", vol: 0.045,
        notes: [[0, 131, 2], [2, 131, 2], [4, 98, 2], [6, 110, 2], [8, 131, 2], [10, 147, 2], [12, 165, 2], [14, 123, 2]],
      },
      { // 王のファンファーレ
        type: "square", vol: 0.024,
        notes: [[0, 523, 2], [2, 659, 2], [4, 784, 3], [7, 659, 1], [8, 587, 2], [10, 523, 2], [12, 587, 4]],
      },
    ],
  },
  // 赤い魂の祠: 不穏な脈動
  shrine: {
    ...
    voices: [
      { // 深く脈打つ低音
        type: "sine", vol: 0.050,
        notes: [[0, 73, 4], [4, 69, 4], [8, 73, 4], [12, 65, 4]],
      },
      { // 揺らぐ呪言
        type: "sine", vol: 0.024,
        notes: [[0, 440, 3], [4, 415, 3], [8, 440, 2], [10, 494, 2], [12, 415, 4]],
      },
    ],
  },
  // ボス: 重く速い脅威のテーマ
...
export function playBgm(name) {
  // 同じ曲が既に鳴っている (または再生待ち) なら途切れさせず継続
  if (name && (bgm.name === name || pendingBgm === name)) return;
  stopBgm();
  if (!name) return;
