...
| `warcry` | 鼓舞 | 敵側全体のATKを上昇（×1.3, 3T） | 約25%発動 | 1〜 | 隊長・主・鼓手 |
| `weaken` | 弱体 | 近接（×0.9）＋プレイヤーのATKを下げる（×0.75, 3T） | 約25%発動 | 1〜 | 呪術師・魔女 |
...
| `magResist` | 魔法耐性 | 攻撃呪文の被ダメを割合カット | 0〜0.9（例 0.5＝5割減） | 魔導生物・結界・霊 |
| `enrage` | 激昂 | HP3割未満で一度だけ ATK×1.5・AGI×1.3 | `true`（1戦1回） | 鬼・獣・主 |
| `endure` | 不屈 | 致死を一度だけ HP1 で耐える | `true`（1戦1回） | 主・大型・執念の不死 |
| `lifesteal` | 吸血 | 与えた物理ダメの割合だけ自己回復 | 0〜1（例 0.3＝3割） | 吸血鬼・寄生・呪樹 |
| `multistrike` | 連撃 | 1手番で続けざまに攻撃（倒れたら次の標的へ） | 2〜4（例 3） | 多腕・双頭・狼 |
| `barrier` | 障壁 | 物理被ダメを半減できる残り回数 | 整数（例 2） | 守護像・番人・結界 |
...
> **実装済み（現行表へ移動済み）**: `warcry` / `weaken` / `magResist` / `enrage` / `endure` / `lifesteal` / `multistrike` / `barrier`。下の提案B表に残っているこれらの行は参考（実体は上の現行表を参照）。残りの未実装は `caster`(提案A) / `slow` / `seal` / `dispel` / `reflect` / `charge` / `firstStrike` / `pierce` / `split` / `deathSummon` / `link` / `disguise`。
