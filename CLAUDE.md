...
=======
- **`souls.js`** — souls/job system data/logic. **One soul per doll** (the old 5-part system was removed): a doll has `soulCls` (active job) and `souls` = `{clsKey: {count, level, exp}}` — per-doll, per-job progression (DQ-style job change at the mansion altar keeps each job's progress). **Collecting more souls of the same class** raises `count`: every copy adds % of base stats (`SOUL_STAT_UP`: common 1% / rare 4% / epic 10% / legend 100%), and crossing `RANKUP_NEED` thresholds (cumulative via `rankThresholds`) raises the job rank 1-5, which lifts the soul-level cap (`SOUL_RANKS[r].cap`: 10/20/30/50/100) and unlocks skills (`JOB_SKILLS` by level, gated by `min(level, rank*10)`) and rank 2-5 passives (`PASSIVES`/`JOB_PASSIVES`; hybrids are removed). Soul **level** grows via the ✦Soul currency (= EXP, earned in battle, auto-distributed by `distributeBattleSoulExp` in game.js and trainable at the altar; cost `soulTrainCost`). Stats = class base × 5 × level factor × rarity × count bonus (`jobStatsOf`). Soul drops roll a class by rarity: normal corpses `rollJobClass` (common70/rare20/epic9/legend1), great corpses `rollGreatJobClass` (rare30/epic50/legend20); acquisition prompts which doll absorbs it (`acquireSoul`/`absorbSoulTo` in game.js).
>>>>>>> origin/main
...
=======
- Save key: `dos-save-v3` in `localStorage` (v1/v2 saves are intentionally orphaned — v3 replaced the 5-part soul system with single-soul per-doll progression). Only the fields in `SAVE_FIELDS` are persisted (transient animation state is excluded).
>>>>>>> origin/main
