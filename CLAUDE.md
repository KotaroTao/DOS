...
- **`combat.js`** — pure combat logic: `Battle` state machine (per-actor AGI-ordered turns), `SPELLS`, `CLASSES`, party/enemy construction, `gainExp`. Returns plain result objects; **all animation/SFX lives in `game.js`** (`combatStep` drives `Battle` and renders the results).
